...
export default function WindSpeedHistoryScreen() {
  ...
  const [windSpeedThreshold, setWindSpeedThreshold] = useState(10.0);
  const [savedWindSpeedThreshold, setSavedWindSpeedThreshold] = useState(10.0);
  ...
  const sliderPanResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderGrant: (evt) => {
        ...
        const value = Math.round((percentage / 100) * 20);
        setWindSpeedThreshold(Math.max(0, Math.min(20, value)));
      },
  ...
  const thresholdPercentage = (windSpeedThreshold / 20) * 100;
  ...
  const loadThresholds = useCallback(async () => {
    ...
    try {
      ...
      if (thresholds.windSpeedThreshold !== undefined) {
        const value = Number(thresholds.windSpeedThreshold);
        setWindSpeedThreshold(value);
        setSavedWindSpeedThreshold(value);
      }
    } catch (error) {
  ...
  const saveThreshold = useCallback(
    async () => {
      ...
      try {
        ...
        setSavedWindSpeedThreshold(windSpeedThreshold);
      } catch (error) {
        ...
        setWindSpeedThreshold(savedWindSpeedThreshold);
      } finally {
        setThresholdSaving(false);
      }
    },
    [selectedFarm?.id, user?.id, windSpeedThreshold, savedWindSpeedThreshold]
  );

  const hasThresholdChanged = windSpeedThreshold !== savedWindSpeedThreshold;
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Wind Speed History</Text>
        ...
        <View style={styles.sectionCard}>
          <Text style={styles.sectionTitle}>Wind Speed Threshold</Text>
          <View style={styles.thresholdContainer}>
            <Text style={styles.thresholdLabel}>
              Alert when wind speed is above: {Math.round(windSpeedThreshold)} km/h
            </Text>
            <View style={styles.sliderContainer}>
              <Text style={styles.sliderMinLabel}>0 km/h</Text>
              <View
              ...
              <Text style={styles.sliderMaxLabel}>20 km/h</Text>
            </View>
            <TouchableOpacity
              style={[
        ...
        <View style={styles.sectionCard}>
          <Text style={styles.sectionTitle}>Wind Speed History</Text>
            ...
            <View style={styles.statisticsContainer}>
              <View style={styles.statisticItem}>
                ...
                <Text style={styles.statisticValue}>
                  {statistics.min !== null ? `${statistics.min.toFixed(1)} km/h` : '--'}
                </Text>
              </View>
              <View style={styles.statisticItem}>
                <Text style={styles.statisticLabel}>Max</Text>
                <Text style={styles.statisticValue}>
                  {statistics.max !== null ? `${statistics.max.toFixed(1)} km/h` : '--'}
                </Text>
              </View>
              <View style={styles.statisticItem}>
                <Text style={styles.statisticLabel}>Average</Text>
                <Text style={styles.statisticValue}>
                  {statistics.average !== null ? `${statistics.average.toFixed(1)} km/h` : '--'}
                </Text>
              </View>
            </View>
          )}
