...
export default function HumidityHistoryScreen() {
  ...
  const [humidityThreshold, setHumidityThreshold] = useState(55.0);
  const [savedHumidityThreshold, setSavedHumidityThreshold] = useState(55.0);
  ...
  const sliderPanResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderGrant: (evt) => {
        ...
        const value = Math.round(20 + (percentage / 100) * 80);
        setHumidityThreshold(Math.max(20, Math.min(100, value)));
      },
  ...
  const loadThresholds = useCallback(async () => {
    ...
    try {
      ...
      if (thresholds.humidityThreshold !== undefined) {
        const value = Number(thresholds.humidityThreshold);
        setHumidityThreshold(value);
        setSavedHumidityThreshold(value);
      }
    } catch (error) {
  ...
  const saveThreshold = useCallback(
    async () => {
      ...
      try {
        ...
        setSavedHumidityThreshold(humidityThreshold);
      } catch (error) {
        ...
        setHumidityThreshold(savedHumidityThreshold);
      } finally {
        setThresholdSaving(false);
      }
    },
    [selectedFarm?.id, user?.id, humidityThreshold, savedHumidityThreshold]
  );

  const hasThresholdChanged = humidityThreshold !== savedHumidityThreshold;
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Humidity History</Text>
        ...
        <View style={styles.sectionCard}>
          ...
          <View style={styles.thresholdContainer}>
            <Text style={styles.thresholdLabel}>
              Alert when humidity is above: {Math.round(humidityThreshold)}%
            </Text>
            <View style={styles.sliderContainer}>
              <Text style={styles.sliderMinLabel}>20%</Text>
              <View
              ...
              >
                <View style={styles.sliderTrack}>
                  <View
                    style={[
                      styles.sliderFill,
                      { width: `${((humidityThreshold - 20) / 80) * 100}%` },
                    ]}
                  />
                </View>
                <View
                  style={[
                    styles.sliderThumb,
                    { left: `${((humidityThreshold - 20) / 80) * 100}%` },
                  ]}
                />
              </View>
            ...
            <View style={styles.statisticsContainer}>
              <View style={styles.statisticItem}>
                ...
                <Text style={styles.statisticValue}>
                  {statistics.min !== null ? `${statistics.min.toFixed(1)}%` : '--'}
                </Text>
              </View>
              <View style={styles.statisticItem}>
                <Text style={styles.statisticLabel}>Max</Text>
                <Text style={styles.statisticValue}>
                  {statistics.max !== null ? `${statistics.max.toFixed(1)}%` : '--'}
                </Text>
              </View>
              <View style={styles.statisticItem}>
                <Text style={styles.statisticLabel}>Average</Text>
                <Text style={styles.statisticValue}>
                  {statistics.average !== null ? `${statistics.average.toFixed(1)}%` : '--'}
                </Text>
              </View>
            </View>
          )}
